            ...
            for (int sideLength = size; sideLength >= 2; sideLength /= 2, range /= 2.0f)
            {
                ...
                for (int x = 0; x < size; x += halfSide)
                {
                    for (int y = (x + halfSide) % sideLength; y < size; y += sideLength)
                    {
                        ...
                        noise[x, y] = avg + (float)(random.NextDouble() * 2.0 - 1.0) * range;

                        if (x == 0) noise[size, y] = avg + (float)(random.NextDouble() * 2.0 - 1.0) * range;
                        if (y == 0) noise[x, size] = avg + (float)(random.NextDouble() * 2.0 - 1.0) * range;
                    }
                }
            }
